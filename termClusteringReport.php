...
if ($autRet->return == 'true'){
	...
	$linkage = 0.5;
	$kappa = 50;
	$gtcr = new getTermClusterReport($overlap, $initialSeed, $finalSeed, $linkage, $kappa);
	$termClusteringReport = $client->getTermClusterReport($gtcr);
	//var_dump($termClusteringReport);
	//print table
	printTable($termClusteringReport);
}else{
...
function printTable($res) {
	echo '<table border="1">';
	foreach ($res->return as $key => $simpleTermClusterRecord) {
		$enrichmentScore = $simpleTermClusterRecord->score;
		$i = $key + 1;
		echo "<tr><td>Annotation Cluster {$i}</td><td colspan=12>EnrichmentScore:{$enrichmentScore}</td></tr>";
		echo '<tr><td>Category</td><td>Term</td><td>Count</td><td>%</td><td>Pvalue</td><td>Genes</td><td>List Total</td><td>Pop Hits</td><td>Pop Total</td><td>Fold Enrichment</td><td>Bonferroni</td><td>Benjamini</td><td>FDR</td></tr>';
		foreach ($simpleTermClusterRecord->simpleChartRecords as $simpleChartRecord) {
			echo "<tr>
					<td>{$simpleChartRecord->categoryName}</td>
					<td>{$simpleChartRecord->termName}</td>
					<td>{$simpleChartRecord->listHits}</td>
					<td>{$simpleChartRecord->percent}</td>
					<td>{$simpleChartRecord->ease}</td>
					<td>{$simpleChartRecord->geneIds}</td>
					<td>{$simpleChartRecord->listTotals}</td>
					<td>{$simpleChartRecord->popHits}</td>
					<td>{$simpleChartRecord->popTotals}</td>
					<td>{$simpleChartRecord->foldEnrichment}</td>
					<td>{$simpleChartRecord->bonferroni}</td>
					<td>{$simpleChartRecord->benjamini}</td>
					<td>{$simpleChartRecord->afdr}</td>
				</tr>";
		}
	}
	echo "</table>";
