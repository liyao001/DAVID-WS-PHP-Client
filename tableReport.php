...
if ($autRet->return == 'true'){
	...
	$sc = new setCategories('BBID,BIOCARTA,COG_ONTOLOGY,GOTERM_BP_FAT,GOTERM_CC_FAT,GOTERM_MF_FAT,INTERPRO,KEGG_PATHWAY,OMIM_DISEASE,PIR_SUPERFAMILY,SMART,SP_PIR_KEYWORDS,UP_SEQ_FEATURE');
	$categorySting = $client->setCategories($sc);
	$categories=explode(',', $categorySting->return);
	//getTableReport
	$tableReport = $client->getTableReport();
	//print table
	var_dump($tableReport);
}else{
