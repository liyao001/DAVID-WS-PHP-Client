...
if ($autRet->return == 'true'){
	...
	$linkage = 0.35;
	$kappa = 50;
	$gcr = new getGeneClusterReport($overlap, $initialSeed, $finalSeed, $linkage, $kappa);
	$geneClusterReport = $client->getGeneClusterReport($gcr);
	//print table
	printTable($idType, $geneClusterReport);
}else{
	exit($autRet->return); //authenticate failed
}

function printTable($idType, $res) {
	echo '<table border="1">';
	foreach ($res->return as $item) {
		echo "<tr><td>{$item->name}</td><td>EnrichmentScore: {$item->score}</td></tr>";
		echo "<tr><td>{$idType}</td><td>Gene Name</td></tr>";
		foreach ($item->listRecords as $subItem) {
			if (is_array($subItem->values)){
				$v = join(',', $subItem->values);
			}else{
				$v = $subItem->values;
			}
			echo "<tr><td>{$v}</td><td>{$subItem->name}</td></tr>";
		}
	}
	echo "</table>";
}
